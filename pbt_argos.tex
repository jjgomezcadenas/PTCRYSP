...
\begin{frame}{In-vivo verification: measuring what was actually delivered}

\vspace{-0.05cm}

\begin{center}
{\small\bfseries
In-vivo verification uses signals generated by the treatment itself to test whether the
proton beam propagated through the patient as expected.
}
\end{center}
...
{\tiny \linespread{0.75}\selectfont
Proton interactions create $\beta^+$ emitters along the beam path.
Their spatial distribution provides an indirect measurement of where irradiation occurred.
}
...
\tiny{
\begin{itemize}
    \item \textbf{Nishio et al. -- BOLPs-RGp, Japan}\\
Gantry-mounted PET used in 48 patients for daily beam-delivery verification.
\href{https://doi.org/10.1016/j.ijrobp.2009.05.065}
{\textit{Int. J. Radiat. Oncol. Biol. Phys.} 76 (2010) 277--286}
\item \textbf{Moglioni et al. -- INSIDE, CNAO}
In-beam PET detected abnormal inter-fractional range variations associated with anatomical changes.
\href{https://doi.org/10.3389/fonc.2022.929949}
{\textit{Front. Oncol.} 12 (2022) 929949}
\item 
\textbf{Zhu et al. -- MGH NeuroPET}
In-room PET enabled acquisition within minutes after treatment, reducing biological washout.
\href{https://pubmed.ncbi.nlm.nih.gov/21677366/}
{\textit{Phys. Med. Biol.} 56 (2011)}
\end{itemize}
...
{\footnotesize\bfseries\color{PTblue}
Prompt-gamma verification
}

\vspace{0.08cm}

{\tiny\linespread{0.75}\selectfont
Prompt $\gamma$ rays are produced immediately during proton interactions,
enabling verification \textbf{while the beam is being delivered}.
}

\vspace{0.12cm}

Hay que corregir un bug en lo que esta comentado
%{\scriptsize\bfseries Clinical and technological progress}
%\tiny{
%\begin{itemize}
%    \item \textbf{Richter et al., 2016}
%First clinical application of prompt-gamma imaging; inter-%fractional range variations were measured during treatment.
%\href{https://pubmed.ncbi.nlm.nih.gov/26774764/}
%  \item  {\textit{Radiother. Oncol.} 118 (2016)}\textbf{Xie et al., 2017} Clinical pencil-beam-scanning study demonstrated millimetre-scale range verification. \href{https://pubmed.ncbi.nlm.nih.gov/28816148/}
%    \item{\textit{Int. J. Radiat. Oncol. Biol. Phys.} 99 (2017)}\textbf{Safavi et al., 2026} Nozzle-mounted Compton-camera system integrated with a clinical proton gantry.\href{https://pubmed.ncbi.nlm.nih.gov/42282050/}
%\end{itemize}
%}
...
{\scriptsize
The objective is not simply to generate another image, but to provide an independent check
...
{\scriptsize
Verification can potentially identify:
}

\vspace{0.04cm}

{\scriptsize\bfseries
Unexpected range shifts

\vspace{0.06cm}
Anatomical changes

\vspace{0.06cm}
Setup or modelling errors

\vspace{0.06cm}
Fractions requiring review or adaptation
}

\vspace{0.15cm}

{\scriptsize
The concept is now moving towards adaptive workflows, where in-vivo verification can act
as a safety layer for deciding when additional imaging or replanning is required.
}
...
{Bertschi et al., \textit{Med. Phys.} 52 (2025) e70142}
}
...
\begin{center}
{\small\bfseries\color{darkred}
The goal is to replace part of today's assumed uncertainty
with patient-specific information from the delivered fraction.
}
\end{center}
...
\begin{center}
{\scriptsize\bfseries
The physics feasibility has been demonstrated.
The remaining challenge is turning a measurement into a clinical decision.
}
\end{center}
...
{\tiny
PET, prompt-$\gamma$ and other techniques can detect
signals correlated with the proton path.
}

\vspace{0.25cm}

{\tiny\bfseries
This has been demonstrated clinically for more than a decade.
}
...
{\tiny
The measured signal is \textbf{not the absorbed dose}.
It must be related to range using physical and anatomical models.
}

\vspace{0.25cm}

{\tiny
A deviation may originate from anatomy, setup, imaging,
beam delivery or modelling.
}
...
{\tiny
A measured range difference does not automatically tell the clinician
whether treatment should continue, be reviewed, re-imaged or replanned.
}
...
{\tiny
A commercial system must fit the gantry and workflow,
analyse data automatically and avoid reducing patient throughput.
}

\vspace{0.25cm}

{\tiny
Clinical benefit and economic value must then justify the added complexity.
}
...
The bottleneck has shifted from detecting the signal
to making the information actionable.
}
\end{center}
...
\href{https://doi.org/10.1016/j.ijrobp.2009.05.065}
{Nishio et al., IJROBP 76 (2010)};
\href{https://pubmed.ncbi.nlm.nih.gov/21677366/}
{Zhu et al., PMB 56 (2011)};
\href{https://doi.org/10.1016/j.radonc.2016.01.004}
{Richter et al., Radiother. Oncol. 118 (2016)};
...
The main barriers now lie in translation into routine clinical practice.
}
\end{center}
...
{\tiny
Detectors must operate close to the patient without interfering with
the gantry, couch or beam angles.
}

\vspace{0.15cm}

{\tiny
PET requires large solid angle; collimated prompt-$\gamma$ systems
...
{\tiny
PET activity or prompt-$\gamma$ emission is
\textbf{not a direct measurement of absorbed dose}.
}

\vspace{0.15cm}

{\tiny
Limited statistics, biological washout, background and model
uncertainties complicate the interpretation of measured deviations.
}
...
{\tiny
Detecting an abnormal range does not automatically determine
...
{\tiny
Current workflows may still require control imaging,
dose recalculation, QA and replanning --- potentially adding
hours or days.
}
...
{\tiny\bfseries
No mature closed-loop adaptive workflow yet.
}
...
{\tiny
Additional hardware and workflow must not reduce patient throughput.
}

\vspace{0.15cm}

{\tiny
Regulatory validation is demanding (FDA \emph{de novo} marcado CE MDR )
\vspace{0.15cm}
Dedicated reimbursement
for range verification is generally absent.
}

\vspace{0.15cm}

{\tiny
Clinical benefit must justify the additional cost and complexity.
}
...
\begin{center}
{\scriptsize\bfseries\color{darkred}
The bottleneck has shifted from measuring the range
to making the measurement clinically actionable and economically sustainable.
}
\end{center}
...
\begin{center}
{\scriptsize\bfseries
A specialised high-value instrumentation market built on new installations,
retrofit of the installed base, and recurring software/service revenues.
}
\end{center}
...
{\tiny
IBA sold 12 rooms in 2025, representing 63\% of global room sales.
}
...
{\tiny
The larger opportunity comes from retrofitting the existing installed base.
}

\vspace{0.16cm}

{\tiny
At a 5\% annual adoption/upgrade rate:
}
...
{\tiny
IBA's 2025 Proton Therapy revenues:
}

\vspace{0.08cm}

{\tiny
\textbf{\EUR\,202.7 M equipment}\\
\textbf{\EUR\,131.7 M services}
}

\vspace{0.16cm}

{\tiny
A monitoring platform could similarly combine:
}
...
{\tiny
plausible mature annual market
}
\end{center}
...
\begin{center}
{\small\bfseries\color{darkred}
Working estimate: \EUR\,10--20 M/year initially,
with potential towards \EUR\,20--30 M/year in a mature market.
}
\end{center}
...
\href{https://www.iba-worldwide.com/sites/default/files/2026-03/iba-fy25-results-presentation-en.pdf}
{IBA FY2025 Results Presentation};
\href{https://www.iba-worldwide.com/sites/default/files/2026-04/iba_5009_ra_2025_coulage_en-web2.pdf}
{IBA Annual Report 2025}.
ASP and retrofit rates are scenario assumptions, not observed market prices.
}
